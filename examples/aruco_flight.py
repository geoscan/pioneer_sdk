...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            corners, ids, rejected_img_points = cv2.aruco.detectMarkers(frame, aruco_dict, parameters=aruco_parameters)
            # If markers are detected
            if np.all(ids is not None):
                # Find center of aruco marker on screen
                x_center = int((corners[0][0][0][0]+ corners[0][0][1][0]+ corners[0][0][2][0]+ corners[0][0][3][0])//4)
                y_center = int((corners[0][0][0][1]+ corners[0][0][1][1]+ corners[0][0][2][1]+ corners[0][0][3][1])//4)
                dot_size = 5
                ...
                r_vec_rodrigues, t_vec, _ = cv2.aruco.estimatePoseSingleMarkers(corners, size_of_marker, camera_mtx, camera_dist)
        ...
        if coordinates is not None:
            ...
            distance = np.sqrt(coordinates[0]**2 + coordinates[1]**2 + coordinates[2]**2)
