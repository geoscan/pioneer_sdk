...
class Pioneer(mavwifi.Wifi):
    ...
    def _message_handler(self):
        while True:
            ...
            if msg is not None:
                ...
                if msg.get_type() == "HEARTBEAT":
                    ...
                    if msg.command == 400 and msg.result_param2 is not None:
                        self._preflight_state.update(
                            BatteryLow=msg.result_param2 & 0b00000001
                        )
                        self._preflight_state.update(
                            NavSystem=msg.result_param2 & 0b00000010
                        )
                        self._preflight_state.update(
                            Area=msg.result_param2 & 0b00000100
                        )
                        self._preflight_state.update(
                            Attitude=msg.result_param2 & 0b00001000
                        )
                        self._preflight_state.update(
                            RcExpected=msg.result_param2 & 0b00010000
                        )
                        self._preflight_state.update(
                            RcMode=msg.result_param2 & 0b00100000
                        )
                        self._preflight_state.update(
                            RcUnexpected=msg.result_param2 & 0b01000000
                        )
                        self._preflight_state.update(
                            UavStartAllowed=msg.result_param2 & 0b10000000
                        )
            ...
            elif self._is_connected and (
                time.time() - self._last_msg_time > self._is_connected_timeout
            ):
                self._is_connected = False
            ...
            while True:
                ...
                if event.is_set():
                    ...
                    if msg.result == 5:  # IN_PROGRESS
                        ...
                        self.log(
                            msg_type=command_name, msg=Pioneer.MAV_RESULT[msg.result]
                        )
                        event.clear()
                    else:
                        self.log(
    ...
    def takeoff(self):
        return self._send_command_long(
            command_name="TAKEOFF", command=mavutil.mavlink.MAV_CMD_NAV_TAKEOFF
        )

    def land(self):
        return self._send_command_long(
            command_name="LAND", command=mavutil.mavlink.MAV_CMD_NAV_LAND
        )
    ...
    def go_to_local_point(self, x, y, z, yaw):
        ...
        self.log(
            msg_type=cmd_name,
            msg=f"sending point {{LOCAL, x:{x}, y:{y}, z:{z}, yaw:{yaw}}} ...",
        )
    ...
    def go_to_local_point_body_fixed(self, x, y, z, yaw):
        ...
        self.log(
            msg_type=cmd_name,
            msg=f"sending point {{BODY_FIX, x:{x}, y:{y}, z:{z}, yaw:{yaw}}} ...",
        )
        mask = 0b0000_10_0_111_111_000  # _ _ _ _ yaw_rate yaw   force_set   afz afy afx   vz vy vx   z y x
        x, y, z = y, x, -z  # ENU coordinates to NED coordinates
        self._point_reached = False
    ...
    def get_local_position_lps(self, get_last_received: bool = False):
        ...
        if "LOCAL_POSITION_NED" in self.msg_archive:
            ...
            msg = msg_dict["msg"]
            if not msg_dict["is_read"].is_set() or (
                msg_dict["is_read"].is_set() and get_last_received
            ):
        ...
        try:
            for confirm in range(self._mavlink_send_number):
                ...
                if event.is_set():
                    ...
                    self.log(
                        msg_type="AUTOPILOT_VERSION",
                        msg=str(
                            [
                                msg.flight_sw_version,
                                msg.board_version,
    ...
    def raspberry_start_capture(self, interval=0.1, total_images=0, sequence_number=0):
        ...
        return self._send_command_long(
            command_name="RPi_START_CAPTURE",
