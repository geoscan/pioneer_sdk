...
class Pioneer:
    ...
    def go_to_local_point(self, x=None, y=None, z=None, yaw=None, callback=None):
        ...
        :param callback: функция или массив функций - выполняются при завершении действия.
        :return: функция ничего не возвращает.
        """
    ...
    def sleep(self, t, callback=None):
        """
        Простая задержка, когда коптер ничего не делает и лишь удерживает свою позицию определенное время.
        :param t: число - время ожидания в секундах
        :param callback: - функция или массив функций - выполняются при завершении действия
        :return: функция ничего не возвращает.
        """
        if self.command_id == 0:
    ...
    def step_inc(self, custom_val=1):
        """
        Увеличивает текущее значение внутреннего счетчика миссий на переданное значение.
        Стандартно увеличивает на 1.
        :param custom_val: число - добавляется к текущему значению счетчика.
        :return: функция ничего не возвращает.
        """
    ...
    def step_dec(self, custom_val=1):
        """
        Уменьшает текущее значение внутреннего счетчика миссий на переданное значение.
        Стандартно уменьшает на 1.
        :param custom_val: число - вычитается из текущего значения счетчика.
        :return: функция ничего не возвращает.
        """
        self.__STEP -= custom_val
    ...
    def step_get(self):
        """
        Возвращает текущее значение счетчика миссий.
        :return: число - текущее значение внутреннего счетчика миссий.
        """
        return self.__STEP

    def step_is(self, val):
        """
        Возвращает True, если текущее значение счетчика миссий равно переданному значению.
        :param val: число - значение для проверки.
        :return: True|False - равен ли счетчик миссий переданному значению.
        """
        if self.__STEP == val:
            return True
        else:
    ...
    def step_reset(self, reset_to_step=0):
        """
        Сбрасывает значение внутреннего счетчика миссий до переданного значения. Стандартно до 0.
        :param reset_to_step: число - значение, до которого осуществзяется сброс.
        :return: функция ничего не возвращает.
        """
